...
class Meeting extends Authenticated_Controller {
    ...
    public function create($project_key = null) {
		...
		Template::set('project_meeting', false);
		...
		if ($this->input->get('on_calendar')) {
            Template::set('project_meeting', false);
        } else {
            Template::set('project_meeting', true);
        }									   
    ...
    public function detail($meeting_key = null) {
        ...
        if (is_numeric($meeting_key)) {
            ...
            $meeting = $this->meeting_model->get_meeting_by_key(
                    $meeting_key, $this->current_user->current_organization_id, 'meetings.*, u.email, u.first_name, u.last_name, u.avatar'
            );
            ...
            if (!$can_view && $meeting->owner_id != $this->current_user->user_id && !in_array(
                            $this->current_user->user_id, array_column($invited_members, 'user_id'))
            ) {
        ...
        if (empty($meeting->is_private)) {
            if (IS_AJAX) {
                echo json_encode([$evaluated, $invited_members, $point_used, $meeting, $agendas, $homeworks]);
                exit;
            }
            ...
            if (IS_AJAX) {
                echo json_encode([$invited_members, $meeting, $agendas, $homeworks]);
                exit;
            }
    ...
    public function update_decider($meeting_key) {
        ...
        //evaluate validation
        if (!is_array($this->input->post('attendee_rate')) || count($this->input->post('attendee_rate')) != $count_members || count($this->input->post('attendee_rate')) < 1) {
            echo json_encode([
                'message_type' => 'danger',
    ...
    public function evaluator($meeting_key) {
        ...
        if ($this->input->post()) {
            ...
            if (($evaluated === false && ($meeting->manage_state == 'evaluate' || $meeting->manage_state == 'decide' || $meeting->manage_state == 'done')) && $role != 'other') {
                if ($role == 'owner') {
                    if (!is_array($this->input->post('attendee_rate')) || count($this->input->post('attendee_rate')) != count($meeting->members)) {
                    ...
                    if (empty($this->input->post('meeting_rate')) || !is_array($this->input->post('agenda_rate')) || count($this->input->post('agenda_rate')) != count($agendas) || count($this->input->post('homework_rate')) != $is_hw_member_count) {
                        $validation_error = true;
                    }
    ...
    public function init_project() {
        ...
        $projects = $this->project_model->select('project_id, name')
                ...
                ->where('status !=', 'draft')
                ->find_all();
		$created_project = $this->project_model->select('project_id, name')
                ->where('organization_id', $this->current_user->current_organization_id)
                ->where('init_mode', 1)
                ->where('status !=', 'draft')
                ...
                ->find_all();
//echo "<pre>";print_r($created_project);die;

        Template::set('data', $data);
        Template::set('projects', empty($projects) ? [] : $projects);
        Template::set('created_project', empty($created_project) ? [] : $created_project[0]);
        Template::render();
    }
        ...
        foreach ($data['meetings'] as $event_id => $meeting) {
            ...
            if (!empty($owner)) {
                ...
                if ($meeting_id === false) {
                    ...
                    if (empty($in_organization_users))
                        $in_organization_users = [];
    ...
    public function get_meeting_alert() {
        ...
        echo json_encode(['html' => $this->mb_project->meeting_alert(false)]);
        exit;
    }
    ...
    public function get_meeting_resource($meeting_id) {
        ...
        echo json_encode(empty($res) ? [] : $res);
        exit;
    }
