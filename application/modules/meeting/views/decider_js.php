...
var updateDeciderInterval = setInterval(update_decider_data, 5000);
...
$(document).on('hide.bs.modal.decider', '#meeting-decider-modal', function(){
	clearInterval(updateDeciderInterval);
...
function update_decider_data()
{	
	var commentOffset = $('#comment-body .list-user-single:last').data('id') 
						? $('#comment-body .list-user-single:last').data('id') 
						: 0;

	$.post('<?php echo site_url('meeting/get_decider_data/' . $meeting_id) ?>', {commentOffset}, function(data) {
		data = JSON.parse(data);
