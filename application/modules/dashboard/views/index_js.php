...
$(document).on('click', '.modal#create-meeting [type=submit]', function(e) {
	e.preventDefault();
	console.log('clicked');
	var url = '<?php echo site_url('meeting/date_picker') ?>';
	if ($('.modal#create-meeting input#meeting-scheduled-start-time').length) {
		url += '?selected_date=' + encodeURIComponent($('.modal#create-meeting input#meeting-scheduled-start-time').val());
	}
